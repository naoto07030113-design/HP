...
| カルテ・会計・その他の管理画面 | 未移行（ブラウザから直接 Supabase） |

次に着手するのは **カルテ（medical-records）領域の API 化**。
患者領域と同じ形（Repository → Service → route、権限＋院スコープ＋監査ログ）で進める。
...
3. 管理画面の残りを API 経由へ移行（medical-records → billing → その他）
...
| 優先 | 内容 |
|---|---|
| P0 | カルテ・会計など未移行の管理画面がブラウザから Supabase を直接操作しており、院をまたいだ制限が効かない（患者領域は解消済み） |
| P0 | RLS が `authenticated` に全テーブル全操作を許可している |
| P0 | 論理削除は患者のみ対応。カルテ・会計・予約は物理削除のまま |
| P0 | バックアップの復元テストが未実施 |
| P1 | `.catch(() => {})` による保存失敗の握りつぶしが管理画面に残っている |
| P1 | ページネーションは患者一覧のみ対応。他の一覧は全件取得のままで、1000行上限に達すると静かに欠落する |
