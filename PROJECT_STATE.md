...
| 会計・その他の管理画面 | 未移行（ブラウザから直接 Supabase） |

次に着手するのは **会計（billing）領域の API 化**。
カルテと同じ形で進める。会計は金額をサーバー側で再計算することが要点。
...
3. 管理画面の残りを API 経由へ移行（billing → その他）
...
| 優先 | 内容 |
|---|---|
| P0 | 会計など未移行の管理画面がブラウザから Supabase を直接操作しており、院をまたいだ制限が効かない（患者・カルテは解消済み） |
| P0 | RLS が `authenticated` に全テーブル全操作を許可している |
| P0 | 論理削除は患者・カルテのみ対応。会計・予約は物理削除のまま |
| P0 | バックアップの復元テストが未実施 |
| P1 | `.catch(() => {})` による保存失敗の握りつぶしが管理画面に残っている |
| P1 | ページネーションは患者・カルテ一覧のみ対応。他の一覧は全件取得のままで、1000行上限に達すると静かに欠落する |
