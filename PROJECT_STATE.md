...
| シフト・ダッシュボード・日計表 | 未移行（ブラウザから直接 Supabase） |

次に着手するのは **シフトとダッシュボードの API 化**、
あるいは **RLS への clinic_id 条件の導入**（DB 側でも院をまたげなくする）。
...
3. 管理画面の残りを API 経由へ移行（シフト → ダッシュボード・日計表）
...
| 優先 | 内容 |
|---|---|
| P0 | シフト・ダッシュボード・日計表がブラウザから Supabase を直接操作しており、院をまたいだ制限が効かない（患者・カルテ・会計・予約は解消済み） |
| P0 | RLS が `authenticated` に全テーブル全操作を許可している |
| P1 | 論理削除は患者・カルテ・会計・予約に対応。シフト等は物理削除のまま |
| P0 | バックアップの復元テストが未実施 |
| P1 | `.catch(() => {})` による保存失敗の握りつぶしが未移行画面（設定・コミュニケーション等）に残っている |
| P1 | ページネーションは患者・カルテ・会計・予約一覧に対応。日計表・ダッシュボードは全件取得のままで、1000行上限に達すると静かに欠落する |
